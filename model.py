...
class Model(nn.Module):
    def __init__(self, input_channels, num_classes, device):
        ...
        self.spectrogram_layer = torchaudio.transforms.Spectrogram(n_fft=self.n_fft, win_length=self.frame_length)
        ...
        self.resnet = ResNet(Block, [3, 4, 6, 3], input_channels, num_classes)
        self.mlp_layer1 = nn.Linear(num_classes * 2, 256)
    ...
    def forward(self, x):
        spectrogram_features = self.spectrogram_layer(x).log()
        ...
        lfcc_hidden_features = self.resnet(lfcc_features)
